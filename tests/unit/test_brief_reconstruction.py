...
def test_structure_and_user_stories():
 ...
 for x in d['briefs']:
  s=(ROOT/x['path']).read_text('utf-8'); assert all(h in s for h in REQ); us=re.findall(r'^- \*\*US-[^:]+:\*\* (.+)$',s,re.MULTILINE); assert len(us)==x['story_count'] and len(us)>=4; assert all(' szeretn' in u and ', hogy ' in u for u in us); assert not any(v in '\n'.join(us) for v in FORBIDDEN)
def test_no_truncated_or_duplicate_stories():
 texts=[]
 for p in B.glob('BRIEF-*.md'):
  texts += re.findall(r'^- \*\*US-[^:]+:\*\* (.+)$',p.read_text('utf-8'),re.MULTILINE)
