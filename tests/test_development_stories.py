...
import pytest
from app.product_service import ProductService,Actor,ProductConflict
...
def test_us_002_preflight_blocks_and_idempotent_export(env):
 s,a,adv,acc,ex,*_=env; good=make(env,confidence=.9); bad=make(env,confidence=.9)
...
def test_us_003_audit_history_is_redacted(env):
 s,a,adv,*_=env;rid=make(env);adv.record_history(a,rid,'receipt.corrected',{'total':1},{'total':2});events=adv.history('t',rid)
...
def test_us_006_ocr_box_provenance(env):
 s,a,adv,*_=env;rid=make(env);adv.store_asset('t',rid,b'PNG','image/png','r.png',[{'text':'10.00','confidence':.9,'x':.1,'y':.2,'width':.2,'height':.1}]);asset=adv.asset('t',rid)
 assert asset and asset.boxes[0]['text']=='10.00'

def test_us_007_inbound_attachment_states(env):
 *_,acc,ex,quality,auto=env; a=env[1];mail=acc.receive_email('t','a@example.com','Receipts',[{'filename':'a.png','content_type':'image/png','size':10},{'filename':'x.exe','content_type':'application/octet-stream','size':5}])
 assert mail['status']=='queued' and len(mail['attachments'])==2

def test_us_008_rule_preview_has_no_mutation(env):
 s,a,adv,acc,ex,quality,auto=env;rid=make(env,vendor='Coffee'); rule=auto.create(a,'Coffee',{'vendor_contains':'Coffee'},{'tags':['meal']},10);before=s.get_receipt(a,rid)
...
def test_us_009_rollback_preserves_later_edits(env):
 s,a,adv,acc,ex,quality,auto=env;r1=make(env);r2=make(env);rule=auto.create(a,'Tag',{}, {'tags':['x']},10);p=auto.preview(a,rule['rule_id'],1);auto.activate(a,rule['rule_id'],1,p['preview_token']);run=auto.run(a,rule['rule_id'],1,[r1,r2]);s.correct(a,r2,{'total':20},2,False)
