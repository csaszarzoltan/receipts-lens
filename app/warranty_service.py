"""Tenant-isolated, idempotent service for FEAT-040."""
from __future__ import annotations
from datetime import UTC,datetime
import hashlib,json,threading,uuid
from app.warranty_models import WarrantyCase,CreateRequest,CommandRequest,Status,NotFoundError,StaleRevisionError,IdempotencyConflictError
...
class WarrantyCaseService:
    ...
    def create(self,tenant_id:str,key:str,request:CreateRequest):
        ...
        with self._lock:
            ...
            item=WarrantyCase(id=uuid.uuid4().hex,tenant_id=tenant_id,status=Status.DRAFT,revision=0,client_reference=request.client_reference,data=request.payload,evidence={"source":"FEAT-040"})
    ...
    def command(self,tenant_id:str,item_id:str,key:str,request:CommandRequest):
        ...
        with self._lock:
            ...
            target={"confirm":"EXPIRED","reject":"CANCELLED","resolve":"EXPIRED","reopen":"DRAFT"}.get(request.action,item.status.value)
