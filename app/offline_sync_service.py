"""Tenant-isolated, idempotent service for FEAT-045."""
from __future__ import annotations
from datetime import UTC,datetime
import hashlib,json,threading,uuid
from app.offline_sync_models import OfflineReceiptDraft,CreateRequest,CommandRequest,Status,NotFoundError,StaleRevisionError,IdempotencyConflictError

class OfflineReceiptDraftService:
    ...
    def create(self,tenant_id:str,key:str,request:CreateRequest):
        ...
        with self._lock:
            ...
            item=OfflineReceiptDraft(id=uuid.uuid4().hex,tenant_id=tenant_id,status=Status.LOCAL_ONLY,revision=0,client_reference=request.client_reference,data=request.payload,evidence={"source":"FEAT-045"})
    ...
    def command(self,tenant_id:str,item_id:str,key:str,request:CommandRequest):
        ...
        with self._lock:
            ...
            target={"confirm":"FAILED","reject":"DISCARDED","resolve":"FAILED","reopen":"LOCAL_ONLY"}.get(request.action,item.status.value)
...
service=OfflineReceiptDraftService()
