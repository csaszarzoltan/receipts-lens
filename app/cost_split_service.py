...
class CostSplitService:
    ...
    def create(self,tenant_id:str,key:str,request:CreateRequest):
        ...
        with self._lock:
            ...
            item=CostSplit(id=uuid.uuid4().hex,tenant_id=tenant_id,status=Status.DRAFT,revision=0,client_reference=request.client_reference,data=request.payload,evidence={"source":"FEAT-043"})
    ...
    def command(self,tenant_id:str,item_id:str,key:str,request:CommandRequest):
        ...
        with self._lock:
            ...
            target={"confirm":"SUPERSEDED","reject":"CANCELLED","resolve":"SUPERSEDED","reopen":"DRAFT"}.get(request.action,item.status.value)
