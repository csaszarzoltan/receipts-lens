...
class BenchmarkRunner:
    def run(self,version:str,cases:list[BenchmarkCase])->BenchmarkReport:
        if not version or not cases: raise ValueError("version and cases required")
        fields=sorted({k for c in cases for k in c.truth})
        exact={f:sum(c.prediction.get(f)==c.truth.get(f) for c in cases)/len(cases) for f in fields}
        tp=sum(c.prediction.get(f)==c.truth.get(f) and c.truth.get(f) is not None for c in cases for f in fields)
        ...
        pairs=[(c.confidence.get(f,0.0),1.0 if c.prediction.get(f)==c.truth.get(f) else 0.0) for c in cases for f in fields]
        ece=sum(abs(conf-ok) for conf,ok in pairs)/len(pairs) if pairs else 0.0
