"""Tenant-isolated, idempotent service for FEAT-044."""
from __future__ import annotations
from datetime import UTC,datetime
import hashlib,json,threading,uuid
from app.savings_models import SpendingGoal,CreateRequest,CommandRequest,Status,NotFoundError,StaleRevisionError,IdempotencyConflictError

class SpendingGoalService:
    ...
    def create(self,tenant_id:str,key:str,request:CreateRequest):
        ...
        with self._lock:
            ...
            item=SpendingGoal(id=uuid.uuid4().hex,tenant_id=tenant_id,status=Status.DRAFT,revision=0,client_reference=request.client_reference,data=request.payload,evidence={"source":"FEAT-044"})
    ...
    def command(self,tenant_id:str,item_id:str,key:str,request:CommandRequest):
        ...
        with self._lock:
            ...
            target={"confirm":"PAUSED","reject":"ARCHIVED","resolve":"PAUSED","reopen":"DRAFT"}.get(request.action,item.status.value)
