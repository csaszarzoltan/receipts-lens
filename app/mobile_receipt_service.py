...
class MobileReceiptJourneyService:
    ...
    def create(self,tenant_id:str,key:str,request:CreateRequest):
        ...
        with self._lock:
            ...
            item=MobileReceiptJourney(id=uuid.uuid4().hex,tenant_id=tenant_id,status=Status.CAPTURING,revision=0,client_reference=request.client_reference,data=request.payload,evidence={"source":"FEAT-048"})
    ...
    def command(self,tenant_id:str,item_id:str,key:str,request:CommandRequest):
        ...
        with self._lock:
            ...
            target={"confirm":"APPROVED","reject":"FAILED","resolve":"APPROVED","reopen":"CAPTURING"}.get(request.action,item.status.value)
...
service=MobileReceiptJourneyService()
