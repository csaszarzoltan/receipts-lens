"""Tenant-isolated, idempotent service for FEAT-042."""
from __future__ import annotations
from datetime import UTC,datetime
import hashlib,json,threading,uuid
from app.refund_models import RefundMatch,CreateRequest,CommandRequest,Status,NotFoundError,StaleRevisionError,IdempotencyConflictError
...
class RefundMatchService:
    ...
    def create(self,tenant_id:str,key:str,request:CreateRequest):
        ...
        with self._lock:
            ...
            item=RefundMatch(id=uuid.uuid4().hex,tenant_id=tenant_id,status=Status.DETECTED,revision=0,client_reference=request.client_reference,data=request.payload,evidence={"source":"FEAT-042"})
    ...
    def command(self,tenant_id:str,item_id:str,key:str,request:CommandRequest):
        ...
        with self._lock:
            ...
            target={"confirm":"REVERSED","reject":"DISPUTED","resolve":"REVERSED","reopen":"DETECTED"}.get(request.action,item.status.value)
